...
We thank the Reviewer for prompting this closer examination of metric-specific reproducibility. After inspecting the three-observer ICCs, we found that low concordance was concentrated in Caliber Uniformity and was image-/ROI-dependent in a subset of cases: residual (case×observer) variance dominated the Caliber ICC, consistent with fragile diameter-variability features rather than with failure of area or topological metrics (which remained high). A related hypothesis that small lesion area alone destabilizes radial caliber sampling received only partial support and does not fully explain the moderate Caliber ICC on the full set.

Accordingly, as a methodological sensitivity analysis, we refined the operationalization of caliber uniformity for reproducibility assessment using existing diameter-CV features already present in the analysis exports: NV Diameter (CV) was Winsorized (pooled 5th–95th percentiles), inverted, and piecewise-scaled to a 0–100 uniformity score, while the least reproducible local-CV component (Local Diameter Variation [max CV%]) was excluded. Under this alternate, robust CV-based proxy, three-rater ICC(2,1) rose from **0.434** for the original Caliber Uniformity Score to **0.765** (95% CI 0.640–0.860; n = 46). Because this proxy is not monotonically aligned with the original Stability/PCA-based Caliber Uniformity Score, we present it as a disclosed sensitivity refinement for caliber-related reproducibility—not as a silent replacement of the manuscript’s primary Caliber Uniformity endpoint. Maturity Index ICC was not improved by this redefinition and remains as reported above.

As a further sensitivity analysis on the 20 most cross-observer–concordant cases (lowest mean z-scored rater range across metrics), ICC(2,1) was 0.864 / 0.950 / 0.852 / 0.924 for area / complexity / caliber uniformity / maturity (details: `icc/icc_multirater_results_subset20.md`); these do not replace the primary n=46 estimates above.
...
- Fill ICC placeholders after analyses (κ filled from `grading/agreement_stats.md`; effect sizes filled from `stats/table3_effect_sizes.md`; **ICC filled from `icc/icc_multirater_results.md`**).
- Decide final title (suggest removing “Pathophysiological” or replacing with “Morphology-Derived Categorization”).
- Suggested title option:  
...
- Figure 1 = existing multi-device examples; Figure 2 = new workflow schematic (to create).
- For expert grading: YY should grade from original OCTA (or unmarked exports) without seeing automated subtype; lock grades before unblinding.
- ICC primary: 3-observer multi-rater ICC(2,1) on **n=46** (done); multilevel VC ICC reported; intra-observer S1/S2 only if data allow.
- Convert this file to PDF for “File 1: response to reviewers” after numbers are filled.
