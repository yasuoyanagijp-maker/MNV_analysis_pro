...
def fast_region_growing(img: np.ndarray, seed: tuple[int, int], a: float, window_size: int = 5) -> np.ndarray:
    ...
    Args:
        img: 入力画像 (グレースケール推奨だがRGBも可)
        seed: シード座標 (x, y)
        a: 閾値の許容係数（長押しで増加させるパラメータ）
        window_size: 局所統計量を計算するための局所ウィンドウのサイズ
        
    Returns:
        np.ndarray: 抽出対象領域が255、背景が0の二値マスク画像
    """
    x, y = seed
    h, w = img.shape[:2]
    ...
    # 3. ポテンシャルマスクの一括生成
    # 画像全体から許容範囲内に収まる全ピクセルを255にする（飛び地も含まれる）
    potential_mask = cv2.inRange(img, low, high)
    
    # 4. シード点からの連結成分抽出 (floodFillを使用)
    # シード位置自体がポテンシャルマスク内で0(許容範囲外)の場合は抽出不可
    if potential_mask[y, x] == 0:
        return np.zeros((h, w), dtype=np.uint8)
        
    # floodFill用のマスク（画像サイズ+上下左右1ピクセル拡張が必要）
    flood_mask = np.zeros((h + 2, w + 2), dtype=np.uint8)
    
    # flood_maskの背景部分(値1)を設定。
    # cv2.floodFillにおいて、maskの非ゼロピクセルは「壁（境界）」とみなされ侵入しない。
    # よって、potential_maskの範囲外(0)の箇所を壁(1)にする。
    flood_mask[1:-1, 1:-1] = np.where(potential_mask == 0, 1, 0).astype(np.uint8)
    
    # flags: 4近傍探索 | flood_maskに「255」を書き込む | マスクのみ更新機能
    flags = 4 | (255 << 8) | cv2.FLOODFILL_MASK_ONLY
    
    # シード点から塗りつぶし実行（image_dummyは変更されない）
    image_dummy = np.zeros((h, w), dtype=np.uint8)
    cv2.floodFill(image_dummy, flood_mask, seed, 0, 0, 0, flags)
    
    # flood_mask上で今回255で塗られた箇所（シードからの連結成分）を抽出結果とする
